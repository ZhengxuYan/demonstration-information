#!/bin/bash
# Train Diffusion Policy lowdim state policy on Transport PH over 10 seeds.
#
# Usage:
#   sbatch --array=1-10%5 scripts/slurm/train_transport_ph_dp_lowdim_10seed_array.sh

#SBATCH --job-name=trans_dp_lowdim
#SBATCH --account=iris
...
#SBATCH --output=/iris/u/jasonyan/slurm/%A_%a_trans_dp_lowdim.out
#SBATCH --error=/iris/u/jasonyan/slurm/%A_%a_trans_dp_lowdim.err
...
TASK_CONFIG="${TASK_CONFIG:-transport_lowdim_abs}"
DATASET_PATH="${DATASET_PATH:-/iris/u/jasonyan/data/diffusion_policy/robomimic/datasets/transport/ph/low_dim_abs.hdf5}"
OUT_ROOT="${OUT_ROOT:-/iris/u/jasonyan/data/diffusion_policy_outputs/transport_ph_lowdim_abs_10seed}"
