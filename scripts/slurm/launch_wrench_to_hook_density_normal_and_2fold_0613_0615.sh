#!/bin/bash
# Submit normal + 2-fold wrench-to-hook density HDF5 export and training jobs.
...
submit_one_dataset() {
  ...
  echo "${dataset_tag} normal_train_job=${normal_job}"
  ...
  for fold in ${FOLDS}; do
    ...
    echo "${dataset_tag} ${fold_tag}_train_job=${fold_job}"
  done
}
