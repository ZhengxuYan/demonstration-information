...
#SBATCH --mem=64GB
#SBATCH --gres=gpu:1
#SBATCH --job-name=square_tf_gmm
#SBATCH --exclude=iris1,iris2,iris3,iris4,iris-hgx-1,iris-hgx-2,iris-hp-z8
#SBATCH --output=/iris/u/jasonyan/slurm/%j_square_tf_gmm.out
#SBATCH --error=/iris/u/jasonyan/slurm/%j_square_tf_gmm.err
...
CONFIG=${REPO}/configs/robomimic/square_mh_bc_transformer_gmm_agent_wrist.json
...
python robomimic/scripts/train.py \
  --config "${CONFIG}" \
  --dataset "${DATASET}" \
  --name square_mh_tf_gmm_agent_wrist_no_object_seed1
