...
#SBATCH --mem=64GB
#SBATCH --gres=gpu:1
#SBATCH --job-name=square_ph_bc_disc
#SBATCH --exclude=iris1,iris2,iris3,iris4,iris-hgx-1,iris-hgx-2,iris-hp-z8
#SBATCH --output=/iris/u/jasonyan/slurm/%j_square_ph_bc_disc.out
#SBATCH --error=/iris/u/jasonyan/slurm/%j_square_ph_bc_disc.err
...
CONFIG=${REPO}/configs/robomimic/square_ph_bc_discrete_wrist.json
...
python robomimic/scripts/train.py \
  --config "${CONFIG}" \
  --dataset "${DATASET}" \
  --name square_ph_bc_discrete_wrist_proprio_seed1_v2
