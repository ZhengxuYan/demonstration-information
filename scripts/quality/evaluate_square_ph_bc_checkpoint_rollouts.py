#!/usr/bin/env python3
"""Evaluate Square PH robomimic BC checkpoints with rollout success sweeps."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--video-every", type=int, default=0, help="write one rollout video every N checkpoints; 0 disables")
...
def evaluate_checkpoint(args: argparse.Namespace, ckpt_path: Path, index: int) -> dict:
...
def main() -> None:
    ...
    for idx, ckpt_path in enumerate(checkpoints):
        ...
        summaries.append(evaluate_checkpoint(args, ckpt_path, idx))
